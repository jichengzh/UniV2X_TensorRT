...
def main():
    ...
    # cfg.model_other_agent_inf.pretrained = None
    ...
    # build multi agent model
    model_multi_agents = MultiAgent(model_ego_agent, model_other_agents)
    fp16_cfg = cfg.get('fp16', None)
    if fp16_cfg is not None:
        wrap_fp16_model(model_multi_agents)
    checkpoint = load_checkpoint(model_multi_agents, args.checkpoint, map_location='cpu')
    if args.fuse_conv_bn:
        model_multi_agents = fuse_conv_bn(model_multi_agents)
    # old versions did not save class info in checkpoints, this walkaround is
    # for backward compatibility
    if 'CLASSES' in checkpoint.get('meta', {}):
        model_multi_agents.model_ego_agent.CLASSES = checkpoint['meta']['CLASSES']
    else:
        model_multi_agents.model_ego_agent.CLASSES = dataset.CLASSES
    ...
    if not distributed:
        ...
        outputs = custom_multi_gpu_test(model_multi_agents, data_loader, args.tmpdir,
                                        args.gpu_collect)
