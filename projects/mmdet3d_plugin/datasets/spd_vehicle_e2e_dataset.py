...
class SPDE2EDataset(NuScenesDataset):
    ...
    def prepare_train_data(self, index, agent_name='ego_vehicle'):
        ...
        for other_agent_name in self.other_agent_names:
            other_agent_data_dict[other_agent_name] = self._prepare_train_data_single(index, agent_name=other_agent_name)

        # UniV2X TODO: hardcode for training with {'img_metas': data['img_metas']}
        if data is None:
            return None
        else:
            return {'ego_agent_data': data, 'other_agent_data_dict': other_agent_data_dict, 'img_metas': data['img_metas']}

    def _prepare_train_data_single(self, index, agent_name='ego_vehicle'):
        """
        ...
        if self.enbale_temporal_aug:
            ...
            if agent_name == 'ego_vehicle':
                ...
                if self.data_infos[first_index]['other_agent_info_dict'][agent_name]['scene_token'] != \
                        self.data_infos[final_index]['other_agent_info_dict'][agent_name]['scene_token']:
                    return None
            # current timestamp
            input_dict = self.get_data_info(final_index, agent_name=agent_name)
        ...
        for i in prev_indexs_list:
            if self.enbale_temporal_aug:
                i = max(0, i)
            input_dict = self.get_data_info(i, agent_name=agent_name)
            if input_dict is None:
                return None
    ...
    def prepare_test_data(self, index):
        data = self._prepare_test_data_single(index, agent_name='ego_vehicle')

        other_agent_data_dict = {}
        for other_agent_name in self.other_agent_names:
            other_agent_data_dict[other_agent_name] = self._prepare_test_data_single(index, agent_name=other_agent_name)

        return {'ego_agent_data': data, 'other_agent_data_dict': other_agent_data_dict}

    def _prepare_test_data_single(self, index, agent_name='ego_vehicle'):
        """
        Training data preparation.
        Args:
            index (int): Index for accessing the target data.
        ...
        for key, value in example.items():
            ...
            elif key == 'inf_track_query':
                data_dict[key] = [DC(value[0])]
            else:
                data_dict[key] = value
        return data_dict
    ...
    def get_data_info(self, index, agent_name='ego_vehicle'):
