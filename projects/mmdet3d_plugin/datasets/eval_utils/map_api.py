...
# 'seaborn-whitegrid' was renamed in matplotlib 3.6+; use fallback gracefully.
try:
    ...
    try:
        ...
        pass  # no grid style available; continue without it

# Define a map geometry type for polygons and lines.
Geometry = Union[Polygon, LineString]

nuscenes_locations = ['singapore-onenorth', 'singapore-hollandvillage', 'singapore-queenstown', 'boston-seaport']
spd_locations = ['yizhuang01', 'yizhuang02', 'yizhuang03', 'yizhuang04', 'yizhuang05', 'yizhuang06', 'yizhuang07',
                         'yizhuang08', 'yizhuang09', 'yizhuang10', 'yizhuang11', 'yizhuang12', 'yizhuang13', 'yizhuang14',
                          'yizhuang15', 'yizhuang16', 'yizhuang17', 'yizhuang18', 'yizhuang19', 'yizhuang20', 'yizhuang21',
                           'yizhuang22', 'yizhuang23', 'yizhuang24', 'yizhuang25', 'yizhuang26', 'yizhuang27', 'yizhuang28']
locations = nuscenes_locations + spd_locations
...
class NuScenesMapExplorer:
    ...
    def __init__(self,
        ...
        if isinstance(self.map_api.canvas_edge, list):
            self.canvas_max_x = self.map_api.canvas_edge[0]
            self.canvas_min_x = 0
            self.canvas_max_y = self.map_api.canvas_edge[1]
            self.canvas_min_y = 0
        else:
            self.canvas_max_x = self.map_api.canvas_edge['max_x']
            self.canvas_min_x = self.map_api.canvas_edge['min_x']
            self.canvas_max_y = self.map_api.canvas_edge['max_y']
            self.canvas_min_y = self.map_api.canvas_edge['min_y']
    ...
    def mask_for_polygons(polygons: MultiPolygon, mask: np.ndarray) -> np.ndarray:
        ...
        # Shapely 2.x removed direct iteration over MultiPolygon; use .geoms
